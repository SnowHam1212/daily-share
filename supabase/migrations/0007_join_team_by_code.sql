...
--
-- ⚠️ 上の「フロントエンドは既にこの RPC を呼び出している」は誤りだった。
-- RPC を呼ぶ変更（4f1bf94）は main の履歴に入っておらず、TeamsTab /
-- TeamSetup は teams を直接 SELECT したままだった。RLS に阻まれて
-- 常に 0 件になるため、招待コード参加は機能していなかった。
-- さらに 0007 という番号が 0007_location_team_sharing と重複していた
-- せいで、本番には関数自体が作られていなかった。
-- 両方とも 0015_restore_join_team_by_code.sql とフロントエンド側の
-- 修正で解消済み。このファイルは履歴として残す（再適用はされない）。
-- ============================================================
